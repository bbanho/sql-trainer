...
const SqlEditor: React.FC<SqlEditorProps> = ({ value, onChange, disabled, fontSize }) => {
  return (
    <div className="w-full h-full flex flex-col border border-ice-300 dark:border-slate-700 bg-ice-100 dark:bg-slate-950 shadow-sm">
      {/* Editor Toolbar - Strictly Utilitarian, NO Traffic Lights */}
      <div className="flex items-center justify-between px-3 py-2 bg-ice-200 dark:bg-slate-900 border-b border-ice-300 dark:border-slate-700">
      ...
      {/* Text Area - Uses Ice-100 (light) or Slate-950 (dark/void) */}
      ...
      {/* Status Bar */}
      <div className="px-3 py-1.5 bg-ice-200 dark:bg-slate-900 border-t border-ice-300 dark:border-slate-700 flex justify-between items-center text-[10px] text-slate-500 font-mono">
        <span>Ln {value.split('\n').length}, Col {value.length}</span>
        <span className={disabled ? 'text-amber-600 dark:text-amber-500 font-bold' : 'text-blue-600 dark:text-blue-500 font-bold'}>
