...
function App() {
  ...
  // CHANGED: Default isErdOpen to true so the diagram is visible immediately
  const [isErdOpen, setIsErdOpen] = useState(true);
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-ice-200 dark:bg-slate-950 font-sans">
      ...
      <header className="bg-slate-900 text-white h-12 flex items-center justify-between px-4 border-b border-slate-700 flex-shrink-0 z-50">
        <div className="flex items-center gap-6">
          ...
          <div className="flex bg-slate-800 p-0.5 rounded-none">
            {SCENARIOS.map(sc => (
              <button
                key={sc.id}
        ...
        <div className="flex items-center gap-4">
           <div className="flex gap-1 bg-slate-800 p-0.5 rounded-none">
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <main className="flex-1 flex flex-col min-w-0 relative bg-ice-200 dark:bg-slate-950">
          ...
          {/* DRAWER */}
          <div className="relative z-0 flex flex-col bg-ice-100 dark:bg-slate-900 border-b border-ice-300 dark:border-slate-800 shadow-inner">
             ...
             <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-full z-20">
               <button onClick={() => setIsErdOpen(!isErdOpen)} className="flex items-center gap-2 px-6 py-1 bg-ice-100 dark:bg-slate-900 border-x border-b border-ice-300 dark:border-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-400 hover:text-blue-600 uppercase tracking-widest shadow-sm">
          ...
          <div className="flex-1 flex flex-col min-h-0 z-10 relative">
            <div className="bg-ice-100 dark:bg-slate-900 border-b border-ice-300 dark:border-slate-800 p-4 flex justify-between items-start">
            ...
            <div className="flex-1 flex overflow-hidden">
              <div className="flex-1 flex flex-col p-4 gap-4 overflow-hidden">
                <div className="flex-none h-48 flex flex-col gap-2">
                  ...
                  <div className="flex items-center gap-0 border border-ice-300 dark:border-slate-700 rounded-none overflow-hidden">
                     <button onClick={handleExecute} disabled={isExecuting || !userSql.trim()} className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-700 text-white px-6 py-2 font-bold text-xs uppercase tracking-wider transition-colors">
                      ...
                      <div className={`flex-1 px-4 py-2 text-xs font-mono uppercase font-bold flex items-center ${
                          ...
                          'bg-ice-100 dark:bg-slate-800 text-slate-500'
                        }`}>
                         {statusMessage.text}
                      </div>
                  </div>
